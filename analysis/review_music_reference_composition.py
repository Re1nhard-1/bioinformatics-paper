"""Restricted-grid audit derived from frozen review_music_external.py, with paired design comparisons."""
...
LEVELS=['balanced','ratio10'];METHODS=['music_weighted','music_nnls'];BUDGETS=[60,300]
def sha(p):
    h=hashlib.sha256()
...
def main():
    ...
    assert run['status']=='completed composition grid; descriptive review pending'
    ...
    for fold in sorted(run['folds'],key=lambda x:x['triple_key']):
        ...
        assert len(f)==11520 and len(diag)==5760 and (f.triple_key==fold['triple_key']).all()
    ...
    key=['case_id','method','mixture_id'];assert len(df)==161280 and not df.duplicated(key).any() and set(df.case_id)==set(cases.index)
    ...
    assert len(dg)==80640 and not dg.duplicated(['case_id','mixture_id']).any()
    ...
    # Unchanged configurations shared with the original ring provide natural replay controls.
    old_run_path=ROOT/'results/music_external/20260917T125429748635Z/run.json'
    old_run=json.loads(old_run_path.read_bytes())
    old_hashes={x['path']:x['sha256'] for x in old_run['artifacts']}
    overlap_tables=[]
    shared_keys=['held_out','triple_key','block','budget','level','dominant_donor','method','mixture_id']
    shared_num=['mae','rmse']+[f'{p}_{k}' for p in ['true','pred'] for k in range(6)]
    for triple in sorted(set(df.triple_key)&{x['triple_key'] for x in old_run['folds']}):
        op=old_run_path.parent/triple/'predictions.csv'
        assert sha(op)==old_hashes[op.relative_to(ROOT).as_posix()]
        old_frame=pd.read_csv(op,keep_default_na=False)
        shared=df[df.triple_key==triple].merge(old_frame,on=shared_keys,suffixes=('_new','_old'),validate='one_to_one')
        maximum=max(float(np.max(abs(shared[x+'_new']-shared[x+'_old']))) for x in shared_num) if len(shared) else 0.0
        assert maximum<=1e-10
        overlap_tables.append(dict(triple_key=triple,shared_prediction_rows=len(shared),maximum_difference=maximum))
    out=ROOT/'results/music_reference_composition_review'/datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ');out.mkdir(parents=True)
    tables={}
    def export(name,frame):
        frame.to_csv(out/(name+'.csv'),index=False,lineterminator='\n');tables[name]=len(frame);return frame
    export('shared_configuration_checks',pd.DataFrame(overlap_tables))
    ...
    bc=export('donor_budget_penalty_contrasts',pd.DataFrame(contrasts));assert len(bc)==28
    ...
    # Preserve all follow-up-vs-original changes, not just the overall mean.
    old_path=ROOT/'results/music_external_review/20260917T133518483857Z/donor_budget_penalty_contrasts.csv'
    old_manifest=json.loads((old_path.parent/'review.json').read_bytes())
    expected_hash=next(x['sha256'] for x in old_manifest['artifacts'] if x['path']==old_path.relative_to(ROOT).as_posix())
    assert sha(old_path)==expected_hash
    old=pd.read_csv(old_path)
    old=old[(old.level=='ratio10')&(old.low_budget==60)&(old.high_budget==300)]
    comp=bc.merge(old,on=['held_out','method','level','low_budget','high_budget'],suffixes=('_followup','_original'),validate='one_to_one')
    assert len(comp)==28
    comp['followup_minus_original_pp']=comp.mae_penalty_change_pp_followup-comp.mae_penalty_change_pp_original
    comp['same_nonzero_sign']=np.sign(comp.mae_penalty_change_pp_followup)==np.sign(comp.mae_penalty_change_pp_original)
    export('paired_design_comparison',comp)
    # Reference/block cases remain dependent; provide them without population inference.
    rb=paired.groupby(['held_out','triple_id','dominant_donor','block','method','budget'],observed=True)[penalties].mean().reset_index()
    export('reference_block_penalties',rb)
    rbc=rb.pivot(index=['held_out','triple_id','dominant_donor','block','method'],columns='budget',values='mae_penalty_pp')
    rbc['I300_minus_I60_pp']=rbc[300]-rbc[60]
    export('reference_block_budget_contrasts',rbc.reset_index())
    db=block.pivot(index=['held_out','block','method'],columns='budget',values='mae_penalty_pp')
    db['I300_minus_I60_pp']=db[300]-db[60]
    export('donor_block_budget_contrasts',db.reset_index())
    export('block_aggregate_budget_contrasts',db.reset_index().groupby(['block','method'],observed=True)[['I300_minus_I60_pp']].mean().reset_index())
    summaries=[]
    ...
    summary={'status':'passed complete reference composition descriptive audit','run':rp.relative_to(ROOT).as_posix(),'run_sha256':sha(rp),
    ...
    summary['original_comparison_source']={'path':old_path.relative_to(ROOT).as_posix(),'sha256':sha(old_path)}
    summary['shared_configuration_checks']=overlap_tables
    summary['followup_minus_original_primary_pp']=signs(comp.loc[comp.method=='music_weighted','followup_minus_original_pp'])
    summary['scope']='One post-outcome alternative donor ring; original cohort, reference prefixes and targets reused. Not new cell draws, independent replication, population inference or equivalence evidence.'
