...
if(mode=="controls"){
  first<-selection$donors[1];dominant<-sort(needed)[1]
  testcases<-cases[cases$held_out==first & cases$triple_id=="0" & cases$block=="3",,drop=FALSE]
  ...
  stopifnot(nrow(chosen)==2L);checks<-list()
  for(i in seq_len(nrow(chosen))){
    ...
    cat("Batch control",i,"/ 2; max difference",difference,"\n")
  }
  jsonlite::write_json(list(status="controls completed",checks=checks,batch_equivalence_passed=all(sapply(checks,function(x)x$equivalence_passed)),
    prediction_rows_per_path=960L,elapsed_seconds=unname(proc.time()["elapsed"]-started)),file.path(out,"controls.json"),auto_unbox=TRUE,pretty=TRUE,digits=16)
}else{
