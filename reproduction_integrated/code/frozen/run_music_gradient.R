# Gradient adapter: original official package unchanged; paired fixed-budget allocations.
...
cases <- cases[cases$held_out == donor & cases$policy == ifelse(mode == "replay", "replay", "gradient"), , drop = FALSE]
...
for (j in seq_len(nrow(cases))) {
  ...
  major <- as.integer(cfg$major_count)
  minor <- as.integer(cfg$minor_count)
  donor_sizes <- table(ref_meta$donor, ref_meta$cell_type)
  expected_sizes <- ifelse(rownames(donor_sizes) == cfg$dominant_donor, major, minor)
  stopifnot(expected_cells == 360L, expected_per_type == 60L,
            ...
            all(donor_sizes == expected_sizes),
            (mode == "replay" || (major %in% c(30L, 40L) && major + 2L * minor == 60L)))
  ...
  for (method in names(methods_to_export)) {
    ...
    frame <- data.frame(case_id = cfg$case_id, held_out = donor, triple_id = as.integer(cfg$triple_id),
      ...
      reference_cells = expected_cells, allocation = cfg$allocation, major_count = major,
      minor_count = minor, dominant_donor = cfg$dominant_donor,
