# Run the unchanged official package; instrumentation records diagnostics only.
...
out <- args[2]
donor <- args[3]
technical <- length(args) >= 4L && args[4] == "technical"
...
cases <- read.delim(file.path(data_dir, "cases.tsv"), check.names = FALSE, colClasses = "character")
...
cases <- cases[cases$held_out == donor, , drop = FALSE]
...
for (j in seq_len(nrow(cases))) {
  ...
  stopifnot(length(ids) == 360L, length(unique(ids)) == 360L,
            !(donor %in% ref_meta$donor), length(unique(ref_meta$donor)) == 3L,
            all(table(factor(ref_meta$cell_type, levels = types)) == 60L))
  ...
  for (method in names(methods_to_export)) {
    ...
    frame <- data.frame(case_id = cfg$case_id, held_out = donor, triple_id = as.integer(cfg$triple_id),
      block = as.integer(cfg$block), scenario = cfg$scenario, ratio = as.integer(cfg$ratio),
...
report <- list(status = "completed", donor = donor, technical_only = technical,
